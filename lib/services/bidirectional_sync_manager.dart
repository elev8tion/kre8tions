...
import 'package:kre8tions/services/code_sync_service.dart' show CodeSyncService, InsertPosition;
...
class BidirectionalSyncManager {
  ...
  final CodeExecutionService _execution = CodeExecutionService.instance;
  ...
  Future<void> updateProperty(String propertyName, dynamic propertyValue) async {
    ...
    try {
      ...
      _debounceTimer = Timer(const Duration(milliseconds: 500), () {
        _execution.analyzeLiveCode(updatedCode, null);
      });
  ...
  Future<void> updateCode(String newCode) async {
    ...
    try {
      ...
      _debounceTimer?.cancel();
      _debounceTimer = Timer(const Duration(milliseconds: 500), () {
        _execution.analyzeLiveCode(newCode, null);
      });
  ...
  Future<void> _parseAndSync() async {
    ...
    try {
      ...
      if (widgetTree != null) {
        // Widget tree is automatically broadcast by the AST parser
  ...
  Future<void> undo() async {
    ...
    // Apply reverse operation based on event type
    // This is a simplified version - full implementation would track reverse operations
